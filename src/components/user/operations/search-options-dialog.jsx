...
import { getOperationsOptionsState, setOperationsOptions } from '@/store/slices/search-slice';

export default function SearchOptionsDialog(props) {
  const { onClose, open } = props;
  const dispatch = useDispatch();
  const searchOptions = useSelector(getOperationsOptionsState);
    ...
    <Dialog onClose={handleClose} open={open} sx={{ p: 2 }}>
      ...
      <Box sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
        <FormControl>
          ...
          <FormGroup sx={{ pt: 0 }}>
            {headerColumns
              .filter(column => column.title !== 'scans') // scans column not used in search
              .map(({ title, label }) => (
                <FormControlLabel
                  key={title}
                  label={label}
