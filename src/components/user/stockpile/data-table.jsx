...
import { getStockpileSearchState } from '@/store/slices/search-slice';
import { getStockpileSelectedDataState, setSelectedStockpile } from '@/store/slices/selected-data-slice';
...
function DataTableToolbar(props) {
  ...
  const message = `Позначено ${numSelected} ${rowsPlural}`;
...
export default function DataTable() {
  const response = useFetchGet(`/api/GET/stockpile`);
  ...
  const search = useSelector(getStockpileSearchState);
  ...
  const searchedRows = rows.filter((item, idx) => {
    ...
    let findedByActualAmount = false;
    let findedByBalanceAmount = false;
    ...
    if (!expr) {
      ...
      findedByCost = true;
      findedByActualAmount = true;
      findedByBalanceAmount = true;
    }
    ...
    if (options.searchByColumns.actualAmount && expr) {
      findedByActualAmount = item.actualAmount.toString().includes(expr);
    }

    if (options.searchByColumns.balanceAmount && expr) {
      findedByBalanceAmount = item.balanceAmount.toString().includes(expr);
    }

    const isFounded = findedByTitle ||
      findedByGroup ||
      ...
      findedByInteger ||
      findedByCost ||
      findedByActualAmount ||
      findedByBalanceAmount;

    return isFounded;
  });

  const filteredRows = searchedRows.filter((item, idx) => {
    ...
    const isFiltered = filteredByCost && filteredByDateOfManufacture;
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      ...
      dispatch(setSelectedStockpile(newSelected));
      return;
    }
    dispatch(setSelectedStockpile([]));
  };
  ...
  return (
    <Box sx={{ width: '100%', height: '99%' }}>
      <Paper sx={{ width: '100%', mb: 2, height: '99%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <Box>
          <DataTableToolbar numSelected={selected.length} title='Запаси' onFilterDialogClick={handleClickFilterDialogkOpen} />
          <TableContainer>
            <Table
              sx={{ minWidth: 750 }}
              ...
              <TableBody>
                {stableSort(filteredRows, getComparator(order, orderBy))
                  ...
                  .map((row, idx) => {
                    ...
                    const labelId = `stockpile-table-item-${row.id}`;
                    ...
                    return (
                      ...
                      >
                        ...
                        {filter.columns.group && <TableCell align="center">{row.group ? row.group.title : ''}</TableCell>}
                        ...
                        {filter.columns.balanceAmount && <TableCell align="center">{ row.balanceAmount}</TableCell>}
                        {filter.columns.actualAmount && <TableCell align="center">{row.actualAmount}</TableCell>}
