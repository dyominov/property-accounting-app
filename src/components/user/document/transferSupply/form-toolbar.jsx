...
const formValidator = async (form) => {
  ...
  const { data, isError } = await fetchGet(`/api/GET/document/transfer-supply?type=${document.type}&number=${document.number}&date=${document.date}`);
  if (data) {
  ...
  let isHolderUnitEmpty = !holder.notSystemUnit;

  if (isHolderNameEmpty) {
    errorMessage('Прізвище, ім`я, по-батькові одержувача не задано');
  }
  if (isHolderUnitEmpty) {
    errorMessage('Підрозділ одержувача не задано');
  }

  const isMainAssetListEmpty = !mainAssets || mainAssets.length === 0;
  const isStockpileListEmpty = !stockpile || Object.keys(stockpile).length === 0;
  if (isMainAssetListEmpty && isStockpileListEmpty) {
    errorMessage('Виберіть хоча б один ОЗ чи запас');
  }

  if (isDocumentNumberEmpty || isDocumetTypeEmpty || isHolderNameEmpty || isHolderUnitEmpty || (isMainAssetListEmpty && isStockpileListEmpty)) return false;
...
function FormToolbar() {
  ...
  const holder = useSelector(getTransferHolderState);
  const mainAssets = useSelector(getTransferredMainAssetsState);
  const stockpile = useSelector(getTransferredStockpileState);
  const document = { header, holder, mainAssets, stockpile };
