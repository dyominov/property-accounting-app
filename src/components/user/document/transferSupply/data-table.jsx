...
import { headerColumns } from '@/setupObjects/documents/transferSupply/headerColumns';
...
import { getTransferredMainAssetsState, getTransferredStockpileState, decrement } from '../../../../store/slices/document-transferSupply-slice';
...
export default function DataTable() {
  ...
  const selectedSupply = useSelector(getTransferredSupplySelectedDataState);

  const responseMainAssets = useFetchGet(`/api/GET/main-assets-selected`, transferredMainAssets);
  const responseStockpile = useFetchGet(`/api/GET/stockpile-selected`, Object.keys(transferredStockpile));
  ...
  if (responseMainAssets.isLoading || responseStockpile.isLoading) {
    return <Box>Loading...</Box>;
  }
  if (responseMainAssets.isError || responseStockpile.isError) {
    return <Box>Error...</Box>;
  }

  const { data: rowsMainAsset } = responseMainAssets;
  const { data: rowsStockpile } = responseStockpile;
  ...
  const rows = rowsMainAsset.map(rMA => ({...rMA, amount: 1, type: 'ma' }))
    .concat(rowsStockpile.map(rSpl => ({ ...rSpl, amount: transferredStockpile[rSpl.id], type: 's'})));
  ...
  return (
    <Box sx={{ width: '100%', height: '99%' }}>
      <Paper sx={{ width: '100%', mb: 2, height: '99%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <Box>
          <DataTableToolbar numSelected={selected.length} title='Передані засоби' />
          <TableContainer>
            <Table
              sx={{ minWidth: 750 }}
