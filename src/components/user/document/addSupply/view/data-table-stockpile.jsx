...
function DataTableToolbar(props) {
  ...
  const rowsPlural = (numSelected === 1) ? 'виріб' : (numSelected >= 2 && numSelected <= 4) ? 'вироби' : 'виробів';
    ...
    >
      {numSelected > 0 ? (
        <Typography
          ...
          component="div"
        >
          Позначено {numSelected} {rowsPlural} запасу
        </Typography>
      ) : (
        <Typography
          sx={{ flex: '1 1 100%' }}
...
export default function DataTableStockpile({ rows }) {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2, height: '99%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <Box>
          <DataTableToolbar numSelected={selected.length} title='Запаси' />
          <TableContainer>
            <Table
              sx={{ minWidth: 750 }}
                  ...
                  .map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="center">{row.group ? row.group.title : ''}</TableCell>
                        <TableCell align="center">{moment(row.dateOfManufacture).format(DATE_FORMAT)}</TableCell>
                        <TableCell align="center">{row.inventoryNumber}</TableCell>
                        <TableCell align="center">{row.serialNumber}</TableCell>
                        <TableCell align="center">{row.balanceAmount}</TableCell>
